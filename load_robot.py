...
for i in range(num_segments):
    ...
    segment_id = p.createMultiBody(baseMass=segment_mass, baseCollisionShapeIndex=segment_col_shape, baseVisualShapeIndex=segment_vis_shape, basePosition=pos)
...
p.createConstraint(robotId, end_effector_index, rope_segments[0], -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0], [0, 0, -segment_length / 2])

# Set a fixed extended target position for the end-effector
targetPos = [0.8, 0.0, 1.0]  # Fixed position to keep the end-effector extended away from the base
