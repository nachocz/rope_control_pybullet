...
class RopeSetup:
    ...
    def create_rope(self, end_effector_position):
        ...
        for i in range(self.num_segments):
            ...
            segment_col_shape = p.createCollisionShape(shapeType=p.GEOM_CAPSULE, radius=self.segment_radius, height=self.segment_length)
            segment_vis_shape = p.createVisualShape(shapeType=p.GEOM_CAPSULE, radius=self.segment_radius, length=self.segment_length, rgbaColor=[1, 0, 0, 1])
            segment_id = p.createMultiBody(baseMass=self.segment_mass, baseCollisionShapeIndex=segment_col_shape, baseVisualShapeIndex=segment_vis_shape, basePosition=segment_pos)

            # Disable collisions between adjacent segments to improve stability
            if i > 0:
                p.setCollisionFilterPair(segment_id, self.rope_segments[i - 1], -1, -1, enableCollision=0)
    ...
    def _attach_to_end_effector(self):
        ...
        p.createConstraint(
            ...
            childFramePosition=[0, 0, -self.segment_length / 2]  # Attach at the bottom of the first rope segment
        )
    ...
    def add_ghost_anchor(self, end_effector_position):
        ...
        self.ghost_anchor_id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=-1, basePosition=ghost_anchor_position)
