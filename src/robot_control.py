...
class RobotControl:
    ...
    def move_end_effector(self, target_position):
        joint_poses = p.calculateInverseKinematics(self.robot_id, self.end_effector_index, target_position)
        for i in range(len(joint_poses)):
            p.setJointMotorControl2(self.robot_id, i, p.POSITION_CONTROL, joint_poses[i])

    def get_end_effector_position(self):
        return p.getLinkState(self.robot_id, self.end_effector_index)[0]  # Returns the position (x, y, z)
