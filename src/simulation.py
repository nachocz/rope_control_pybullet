...
class Simulation:
    ...
    def run(self, trajectory: TrajectoryBase):
        ...
        rope_setup = RopeSetup(
            self.config["rope"],
            self.robot_control.robot_id,
            self.robot_control.end_effector_index,
        )
        ...
        for _ in range(
            int(stabilization_time / self.config["simulation"]["time_step"])
        ):
            p.stepSimulation()
            time.sleep(self.config["simulation"]["time_step"] / 10)
        ...
        while t < self.config["simulation"]["duration"]:
            ...
            current_position, _ = p.getLinkState(
                self.robot_control.robot_id, self.robot_control.end_effector_index
            )[:2]
