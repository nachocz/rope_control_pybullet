...
robotId = p.loadURDF("franka_panda/panda.urdf", basePosition=robot_start_pos, baseOrientation=robot_start_ori, useFixedBase=True, flags=p.URDF_USE_SELF_COLLISION)

# Initialize robot's joints to a favorable starting position (bent configuration to avoid singularities)
initial_joint_positions = [0.0, -0.5, 0.3, -1.0, 0.0, 1.0, 0.785]  # Adjusted to keep the arm bent
...
for i in range(num_segments):
    ...
    segment_col_shape = p.createCollisionShape(shapeType=p.GEOM_CAPSULE, radius=segment_radius, height=segment_length)
    segment_vis_shape = p.createVisualShape(shapeType=p.GEOM_CAPSULE, radius=segment_radius, length=segment_length, rgbaColor=[1, 0, 0, 1])
    segment_id = p.createMultiBody(baseMass=segment_mass, baseCollisionShapeIndex=segment_col_shape, baseVisualShapeIndex=segment_vis_shape, basePosition=segment_pos)

    # Disable collisions between adjacent segments for stability
    if i > 0:
        p.setCollisionFilterPair(segment_id, rope_segments[i - 1], -1, -1, enableCollision=0)
...
end_effector_constraint = p.createConstraint(
    robotId, end_effector_index, rope_segments[0], -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0], [0, 0, -segment_length / 2]
)

# Create a ghost anchor point at the loose end of the rope
ghost_anchor_position = [targetPos[0], targetPos[1], targetPos[2] - (num_segments + 1) * segment_length]
ghost_anchor_id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=-1, basePosition=ghost_anchor_position)
...
with open("robot_states_log.csv", mode='w', newline='') as robot_log_file, \
     open("rope_states_log.csv", mode='w', newline='') as rope_log_file:

    robot_writer = csv.writer(robot_log_file)
    rope_writer = csv.writer(rope_log_file)

    # Headers for robot states log
    robot_headers = ['time', 'end_effector_x', 'end_effector_y', 'end_effector_z',
    ...
    while t < simulation_duration:
        ...
        current_target_pos = [targetPos[0] + dx, targetPos[1], targetPos[2]]  # Y and Z are kept fixed

        # Calculate the inverse kinematics for the current target position
        jointPoses = p.calculateInverseKinematics(robotId, end_effector_index, current_target_pos)
        ...
        link_state = p.getLinkState(robotId, end_effector_index, computeLinkVelocity=True)
        end_effector_pos = [round(x, 4) for x in link_state[0]]  # Position [x, y, z]
        end_effector_ori = [round(x, 4) for x in link_state[1]]  # Orientation as quaternion [qx, qy, qz, qw]
        end_effector_lin_vel = [round(x, 4) for x in link_state[6]]  # Linear velocity [vx, vy, vz]
        end_effector_ang_vel = [round(x, 4) for x in link_state[7]]  # Angular velocity [angular_vx, angular_vy, angular_vz]

        # Collect joint angles
        joint_data = [round(x, 4) for x in jointPoses[:7]]

        # Write robot states to CSV
        robot_writer.writerow([round(t, 4), *end_effector_pos, *end_effector_ori, *end_effector_lin_vel, *end_effector_ang_vel, *joint_data])
